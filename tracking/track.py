    ...
    for r in results:
        ...
        if img is not None:
            ...
            if args.save:
                ...
                if not hasattr(yolo, 'output_writer'):
                    height, width = img.shape[:2]
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  
                    yolo.output_writer = cv2.VideoWriter(
                        f"{args.project}/{args.name}.mp4", fourcc, 30, (width, height)
                    )
                yolo.output_writer.write(img)

        frame_time = time.time() - frame_start_time
        time.sleep(max(0, 1/30 - frame_time)) 

    if args.save and hasattr(yolo, 'output_writer'):
        yolo.output_writer.release()
